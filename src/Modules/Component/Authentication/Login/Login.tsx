...
export default function Login() {
  ...
  return (
    <div className="container-fluid d-flex justify-content-center align-items-center p-5">
      ...
      <h3 style={{color:"#EF9B28"}} className="pb-5">Login</h3>
      <form onSubmit={handleSubmit(onsubmit)}>
      <div className="mb-3">
  <label  className="form-label" style={{color:"#EF9B28"}}>E-mail </label>
  <input type="email" className="Form-control"  placeholder="Enter your E-mail"{...register("email",EmailValidation)}/>
  {errors.email && <span className="text-danger">{errors.email.message}</span>}
          </div>
          <div className="mb-3">
  <label  className="form-label" style={{color:"#EF9B28"}}>Password </label>
  <span  className="input-group text-white  d-flex justify-content-end align-items-center mb-2 " onMouseUp={(e)=>e.preventDefault()}
    onMouseDown={(e)=>e.preventDefault()}
    onClick={()=>Setshowpassword(!showpassword)}
    >
<i className={`fa-regular ${showpassword ? "fa-eye" : "fa-eye-slash"}`}></i>
</span>
  <input type={`${showpassword?"text":"password"}`} className="Form-control"  placeholder="Enter your password" {...register("password",PasswordValidation)}/>
